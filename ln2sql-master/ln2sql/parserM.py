...
class SelectParser(Thread):
    def __init__(self, columns_of_select, tables_of_from, phrase, count_keywords, sum_keywords, average_keywords,
        ...
        print(f'In this iteration, columns_of_select is {columns_of_select},tables_of_from is {tables_of_from}, phrase is {phrase},'
              f'count_keywords is {count_keywords},sum_keywords is {sum_keywords}, average keywords is {average_keywords},'
              f'max_keywords is {max_keywords}, min_keywords is {min_keywords}, distinct_keywords is {distinct_keywords},'
              f'database_dico is {database_dico}, database_object is {database_object}')
        print('\n\n')

    def get_tables_of_column(self, column):
        print('-----GET_TABLES_OF_COLUMN()-----', datetime.now())
        tmp_table = []
        for table in self.database_dico:
            print(f'In self.get_tables_of_column(), `for {table} (table) in self.database_dico`')
            print(f'Also here, checking for equality that if {column}(column) in {self.database_dico[table]}(self.database_dico[table])')
            if column in self.database_dico[table]:
                tmp_table.append(table)
        print(f'Finally the list tmp_table which is being returned is {tmp_table}')
        print('-----GET_TABLES_OF_COLUMN()-----')
        print('\n\n')
        return tmp_table

    def get_column_name_with_alias_table(self, column, table_of_from):
        print('someshit')
        print('-----GET_COLUMN-NAME-WITH-ALIAS-TABLE()-----', datetime.now())
        one_table_of_column = self.get_tables_of_column(column)[0]
        tables_of_column = self.get_tables_of_column(column)
        print(f'Here the variables one_table_of_column is {one_table_of_column}, tables_of_column is {tables_of_column}.\nThey are just defined upon the list returned by GET_TABLES_OF_COLUMN()')
        if table_of_from in tables_of_column:
            retObj = str(table_of_from) + '.' + str(column)
            print(f'If {table_of_from} (table_of_from) in {tables_of_column} (tables_of_column), returned value is {retObj} (str(table_of_from) + "." + str(column))')
            print('-----GET_COLUMN-NAME-WITH-ALIAS-TABLE()-----')
            return retObj
        else:
            retObj = str(one_table_of_column) + '.' + str(column)
            print(f'If {table_of_from} (table_of_from) not in {tables_of_column} (tables_of_column), returned value is {retObj} (str(one_table_of_column) + "." + str(column))')
            print('-----GET_COLUMN-NAME-WITH-ALIAS-TABLE()-----')
            return retObj

    def uniquify(self, list):
        print('-----UNIQUIFY()-----', datetime.now())
        print(f'One parameter list is fed with {list}, which is iterated with a for loop')
        print('If the iterated element not in an empty list named `already`, that element will be appended to the `already` list.')
        print("It's an empty list anyways")
        already = []
        for element in list:
            if element not in already:
                already.append(element)
        print(f'The returned list `already` is {already}')
        print('-----UNIQUIFY()-----')
        return already
